...
void Dispatcher::RegisterBusinessUnitTransaction(int transactionid, BusinessUnit* bu)
{
    transactionid2bu_[transactionid].insert(bu);
}

void Dispatcher::RegisterBusinessUnitEvent(int eventid, BusinessUnit* bu)
{
    eventid2bu_[eventid].insert(bu);
}

void Dispatcher::RegisterBusinessUnitTimer(int interval, BusinessUnit* bu)
{
    if (bu)
    {
...
void Dispatcher::RegisterLightService(std::shared_ptr<LightService>& service)
{
    lightservices_[service->serviceid()] = service;
}

void Dispatcher::DispatchTransaction(participant_ptr participant, transaction_ptr trans)
{
...
void Dispatcher::SendTransaction(int sessionid, transaction_ptr trans)
{
    if (sessionid)
    {
...
void Dispatcher::DispatchEvent(participant_ptr participant, int eventid)
{
