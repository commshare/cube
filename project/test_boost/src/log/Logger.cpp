...
namespace light
{
    ...
    public:
        ...
        // 异步分等级输出日志
        void InitLevelSink(const std::string & filename);

        // init console sink.
        template<typename sink_type>
        void InitConsoleSink(SeverityLevel sev);
    ...
    void Logger::impl::InitLevelSink(const std::string & filename)
    {
        for (int i = trace; i <= critical; ++i) {
            std::stringstream allfilename;
            allfilename << "logs\/" << filename 
                << "_" << (SeverityLevel)i << "_%Y%m%d_%H%M%S.log";
            ...
            boost::shared_ptr<sinks::asynchronous_sink < boost::log::sinks::text_file_backend > > sink(new sinks::asynchronous_sink < sinks::text_file_backend >(file_backend));
    ...
    template<typename sink_type>
    void Logger::impl::InitConsoleSink(SeverityLevel sev)
    {
        ...
        console_sink->set_filter(attr_severity >= sev);
    ...
    void Logger::InitLevelLog(const std::string & filename)
    {
        m_impl->InitLevelSink(filename);
    }

    void Logger::InitConsoleLog(SeverityLevel sev)
    {
        m_impl->InitConsoleSink<impl::sync_console_sink>(sev);
    }
