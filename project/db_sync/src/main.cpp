...
int main(int argc, char **argv)
{
    ...
    if (!base::default_log_binder::bind_trace(log_config_file.c_str())) {
        cout << "db sync server bind trace failed" << endl;
    }
    base::trace::enable_std_output(output_log_to_console);

    TRACE_SYSTEM(MODULE_NAME,"main function to be running %d:%s", 110, "hello db_sync");
